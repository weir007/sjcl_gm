...
// 压缩函数
function CF(V, Bi) {//V and Bi are bitArray or Uint32Array
  // 消息扩展
  //const wordLength = 32;
  // 将消息分组B划分为16个字W0， W1，…… ，W15 （字为长度为32的比特串）
  const W = sjcl.bitArray.bitSlice(Bi, 0, 512/*32*16*/);
  const M = [];// W'
  // W[j] <- P1(W[j−16] xor W[j−9] xor (W[j−3] <<< 15)) xor (W[j−13] <<< 7) xor W[j−6]
  //wr: To keep capability to old version browsers, es5+ should better be avoided
  for (let j = 16; j < 68; j += 1) {
    W.push(P1(W[j - 16] ^ W[j - 9] ^ rol(W[j - 3], 15)) ^ rol(W[j - 13], 7) ^ W[j - 6]);
  }
  ...
  // 压缩
  const wordRegister = sjcl.bitArray.bitSlice(V, 0, 256);// 字寄存器
/*  for (let j = 0; j < 8; j += 1) {
...
*/
  ...
  let TT2;
  for (let j = 0; j < 64; j += 1) {
    SS1 = rol((rol(A, 12) + E + rol(T(j), j)), 7);//overflow?
    SS2 = SS1 ^ rol(A, 12);

    TT1 = FF(A, B, C, j) + D + SS2 + M[j];
