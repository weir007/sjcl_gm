...
//////////////////Debug
  console.log('_h : '+binary2hex(A)+' '+binary2hex(B)+' '+binary2hex(C)+' '+binary2hex(D)+' '+binary2hex(E)+' '+binary2hex(F)+' '+binary2hex(G)+' '+binary2hex(H));
  // 中间变量
  let SS1;
  let SS2;
  let TT1;
...
//console.log('SS1['+j+']='+binary2hex(SS1)+' SS2['+j+']='+binary2hex(SS2));

    TT1 = calMulti(add)(FF(A, B, C, j), D, SS2, M[j]);
    TT2 = calMulti(add)(GG(E, F, G, j), H, SS1, W[j]);
//alert('his GG=>'+binary2hex(GG(E, F, G, j)));
//console.log('TT1['+j+']='+binary2hex(TT1)+' TT2['+j+']='+binary2hex(TT2));
//console.log('W['+j+']='+binary2hex(W[j])+' M['+j+']='+binary2hex(M[j]));
    ...
    F = E;
    E = P0(TT2);
  //console.log('V'+j+':'+binary2hex(A)+' '+binary2hex(B)+' '+binary2hex(C)+' '+binary2hex(D)+' '+binary2hex(E)+' '+binary2hex(F)+' '+binary2hex(G)+' '+binary2hex(H));
  }
...
function sm3(str) {
  ...
  //const binary = '011000010110001001100011';
  // 填充
  ...
  for (let i = 0; i <= n - 1; i += 1) {
    ...
    console.log('W'+i+':'+binary2hex(B));
	V = CF(V, B);
	console.log('V'+i+':'+binary2hex(V));
  }
  return binary2hex(V);
}